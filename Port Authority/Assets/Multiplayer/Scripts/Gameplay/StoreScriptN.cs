...
public class StoreScriptN : NetworkBehaviour
{
    [Header("UI")]
    ...
    public static StoreScriptN Instance;
    ...
    {
        print("score: " + ScoreManagerN.Instance.GetSpendableScore());
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= currentDurabilityCost
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= currentSpeedCost
                && currentSpeedLevel <= maxSpeedLevel)
        {
            ...
            LineFollowN.globalBoatSpeed = LineFollowN.baseBoatSpeed * increaseFactor;
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= bigCargoShipCost
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= biggerCargoShipCost
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= whiskeyCost
                && !whiskeyPurchased)
        {
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= furnitureCost
                && !furniturePurchased)
        {
    ...
    {
        if (
                ScoreManagerN.Instance.GetSpendableScore() >= industrialEquipmentCost
